    ...
    kynamicTree: {
        ...
        setting: {
            ...
            callback: {
                ...
                click: function(event, treeId, treeNode){
                    ...
                    $.post("/LanDiskFolder/showVersionsById", parameter, function(data){
        ...
        addNode: function(addJSON){
            ...
            if (fileName != null) {//fileName不为null
                if (fileName != "") {
                    //检查是否有重名的现象
                    $.post("/LanDiskFolder/isExsitName", {
                        name: fileName
                    }, function(data){
                        if (data.message == "1") {//重名了
                            ...
                            var parameter = {
                                ...
                                pid: kynamic.kynamicTree.pNode.id
                            };
                            $.post("/LanDiskFolder/saveKynamic", parameter, function(data2){
        ...
        deleteNode: function(){
            ...
            if (kynamic.kynamicTree.pNode.isParent) {//文件夹节点
                if (kynamic.kynamicTree.zTree.getNodeByParam("pid", kynamic.kynamicTree.pNode.id)) {
                    alert("因为有子节点，所以不能删除");
                }
                else {
                    if (window.confirm("您确认要删除吗?")) {
                        ...
                        //判断该节点是否有兄弟节点
                        $.post("/LanDiskFolder/showSiblingNodes", pararmeter, function(data){
                            if (data.kynamicList.length < 2) {//没有兄弟节点
                                //kynamic.kynamicTree.zTree.getNodeByParam("id",kynamic.kynamicTree.pNode.pid);
                                $.post("/LanDiskFolder/showParentNode", pararmeter, function(data2){
                                    var pNode = kynamic.kynamicTree.zTree.getNodeByParam("id", data2.kynamic.id);//获取父节点
                                    $.post("/LanDiskFolder/deleteNode", pararmeter, function(data3){
        ...
        updateNode: function(){
            ...
            $.post("/LanDiskFolder/isExsitName", pararmeter, function(data){
                if (data.message == "1") {//重名了
                    alert("该文件名称已经存在，请重新输入");
                }else{//进行修改
                    $.post("/LanDiskFolder/updateKynamic",para,function(data2){
    ...
    version: {
        ...
        showVersionsByKID: function(versionList){
            ...
            for (var i = 0; i < versionList.length; i++) {
                ...
                $versionA.attr("updatetime",getMyDate(updatetime));
